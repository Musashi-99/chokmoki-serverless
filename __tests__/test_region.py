...
from src.models.region import available_regions, is_valid_region, normalize_region_code
...
def test_order_region_enforcement_helpers():
    ...
    def principal(**overrides):
        defaults = dict(
            ...
            scopes=frozenset({"orders:read"}), region=None, is_root=False,
        )
    ...
    # No region on the principal (global admin) -> never restricted.
    global_admin = principal(region=None)
    _enforce_order_region(global_admin, order_au)  # must not raise
    assert _region_filter(global_admin) is None

    # Root is never restricted even if a region happens to be set.
    root = principal(region="IN", is_root=True)
    ...
    default_admin = principal(region="Default")
